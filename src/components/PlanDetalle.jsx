...
const PlanDetalle = ({ planes }) => {
  ...
  const formularioCompleto = fechaSeleccionada && horaSeleccionada && nombreCliente.trim() !== "" && correoCliente.includes("@");
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/90 backdrop-blur-md rounded-[3rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row border border-rose-100/50">
          ...
          {/* SECCIÓN DE RESERVA */}
          <div className="lg:w-1/2 bg-rose-50/40 p-8 lg:p-12 border-l border-rose-100 flex flex-col gap-6">
            
            {/* 1. EL CALENDARIO */}
            <div>
              <h3 className="text-rose-900 font-bold mb-3 uppercase tracking-widest text-xs flex items-center gap-2">
                <span className="bg-rose-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]">1</span>
                Elige tu fecha:
              </h3>
              <div className="bg-white p-3 rounded-2xl shadow-inner border border-rose-100 scale-95 origin-top">
            ...
            <AnimatePresence>
              {fechaSeleccionada && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}>
                  <h3 className="text-rose-900 font-bold mb-3 uppercase tracking-widest text-xs flex items-center gap-2">
                    <span className="bg-rose-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]">2</span>
                    Horario:
                  </h3>
            ...
            {/* 3. DATOS PERSONALES (NUEVO) */}
            <div className="space-y-3">
              <h3 className="text-rose-900 font-bold mb-1 uppercase tracking-widest text-xs flex items-center gap-2">
                <span className="bg-rose-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px]">3</span>
                Tus datos:
              </h3>
            ...
            <div className="mt-4">
              <button
                ...
                className={`w-full py-4 rounded-2xl font-black text-lg shadow-xl transition-all flex items-center justify-center gap-3 ${
                  formularioCompleto
                  ? 'bg-[#25D366] text-white hover:bg-[#20ba5a] hover:scale-[1.02] cursor-pointer'
